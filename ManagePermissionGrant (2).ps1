...
# ########################################################
# Environment restriction configuration
# This script is only authorized to run against the Power Platform
# environment named below. It must NOT be applied globally / to any
# other Power Platform environment.
# ########################################################
$AllowedPowerPlatformEnvironmentName = "AI_Factory"
...
Write-Host "#"
Write-Host "# RESTRICTED SCOPE: this script is only authorized to run for the"
Write-Host "# Power Platform environment '$AllowedPowerPlatformEnvironmentName'. It must not be run against any other environment."
Write-Host "########################################################"
...
{
	Install-Module -Name Microsoft.PowerApps.Administration.PowerShell -Scope CurrentUser -WarningAction Ignore -Force
}
...
# ########################################################
# Power Platform environment validation
# This gate confirms the operator is targeting the '$AllowedPowerPlatformEnvironmentName'
# environment before any Entra ID / permission grant change is made.
# The script exits immediately if the environment cannot be confirmed.
# ########################################################
Write-Host "Connecting to Power Platform to validate the target environment..."
try
{
...
{
	Write-Warning "Unable to connect to Power Platform Administration. Execution terminated. Error: $($_.Exception.Message)"
	Exit
}

$candidateEnvironments = Get-AdminPowerAppEnvironment | Where-Object { $_.DisplayName -eq $AllowedPowerPlatformEnvironmentName }

if (-not $candidateEnvironments)
{
	Write-Warning "VALIDATION FAILED: the Power Platform environment '$AllowedPowerPlatformEnvironmentName' was not found, or you do not have access to it."
	Write-Warning "This script is only authorized to run against '$AllowedPowerPlatformEnvironmentName'. Execution terminated."
	Exit
}

if (@($candidateEnvironments).Count -gt 1)
{
	Write-Warning "VALIDATION FAILED: more than one Power Platform environment named '$AllowedPowerPlatformEnvironmentName' was found. Cannot unambiguously confirm the target environment. Execution terminated."
	Exit
}

$targetEnvironment = $candidateEnvironments
Write-Host "The following Power Platform environment was matched:"
$targetEnvironment | Select-Object DisplayName, EnvironmentName | Format-Table -wrap -auto

$confirmedEnvironment = $targetEnvironment | Select-Object DisplayName, EnvironmentName | Out-GridView -Title "Confirm the target Power Platform environment (must be '$AllowedPowerPlatformEnvironmentName')" -OutputMode Single

if (-not $confirmedEnvironment -or $confirmedEnvironment.DisplayName -ne $AllowedPowerPlatformEnvironmentName)
{
	Write-Warning "VALIDATION FAILED: the confirmed environment does not match the authorized environment '$AllowedPowerPlatformEnvironmentName'."
	Write-Warning "This script must not be applied globally or to any other Power Platform environment. Execution terminated."
	Exit
}

Write-Host "Validation passed. This execution is confirmed to be scoped to the Power Platform environment '$AllowedPowerPlatformEnvironmentName' ($($targetEnvironment.EnvironmentName))."
Write-Host "Note: the oAuth2PermissionGrant object created below is an Entra ID (tenant-level) object. This validation gate exists to ensure the operator explicitly confirms they intend this change to support '$AllowedPowerPlatformEnvironmentName' only, and does not proceed for any other environment."
...
Disconnect-MgGraph
Disconnect-PowerApps -ErrorAction Ignore
Write-Host "Script execution completed successfully, scoped to Power Platform environment '$AllowedPowerPlatformEnvironmentName' ($($targetEnvironment.EnvironmentName))."
